...
public class AuthorController {
    ...
    @GetMapping("")
    public ResponseEntity<ApiResponse<Page<AuthorResponse>>> findAllAuthors(@PageableDefault(page = 0, size = 10) Pageable pageable) {
        return ResponseEntity.status(HttpStatus.OK).body(ApiResponse.success(authorService.getAllAuthors(pageable)));
    }
    @GetMapping("{id}")
